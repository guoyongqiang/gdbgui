...
class Locals extends React.Component {
    ...
    render(){
        ...
        for (let local of sorted_local_objs){
            ...
            if(obj){
                content.push(<GdbVariable obj={obj} key={obj.expression} expression={obj.expression} expr_type='expr' />)
            }else{
                content.push(<GdbVariable obj={local} key={local.name} expression={local.name} expr_type='local' />)
            }
        }
